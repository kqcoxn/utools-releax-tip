...
TS["TimingStore<br/>timingState, focusTime, relaxTime, laterTime"]
...
- TimingStore：负责计时状态与计时器生命周期，与 StatusStore 协作以控制窗口显示与通知行为。
...
- [timingStore.ts:22-140](file://src/stores/timingStore.ts#L22-L140)
...
- 计时器逻辑
  ...
  - 在 relax 状态每 10 秒检查一次，必要时调用 ut.showWindow(isOnWindow)。
- 与 StatusStore 的交互
...
## 结论
StatusStore 作为界面状态的核心，通过 isOnWindow 与 operationPanel 的组合，实现了窗口显示与面板展开的精细化控制。它与 TimingStore、SettingsStore、HitokotoStore 协同，结合 Utools 与 TimerService，构建了完整的界面状态管理闭环。通过合理的动画与计时器优先级策略，既保证了性能，也提升了用户体验。后续扩展应遵循“单一职责、最小耦合、明确边界”的原则，确保状态一致性与可维护性。
